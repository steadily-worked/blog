...
서버는 클라이언트로부터 수신하는 이벤트를 다른 유저들에게 relay하고 있기 때문에 서버로부터 오는 `JoinWorkSpace` 이벤트는 해당 워크스페이스에 진입해 있는
...
각 세그먼트는 한 명의 유저만 점유할 수 있고, 점유 시 다른 유저들에게는 Lock된 상태로 수정 불가능한 형태로 보여져야 했습니다. 이를 위해 다음과 같은 상태 구조로 관리했습니다:

```tsx
// 활성 유저 목록 (Set으로 중복 방지 및 O(1) 추가/삭제)
const [activeUsers, setActiveUsers] = useState<Set<string>>(new Set());

// 유저별 점유 세그먼트 매핑 (accountId -> segmentId)
const [lockedSegments, setLockedSegments] = useState<Map<string, string>>(
  new Map(),
);

// 세그먼트별 점유자 매핑 (segmentId -> accountId) - 빠른 조회용
const [segmentOwners, setSegmentOwners] = useState<Map<string, string>>(
  new Map(),
);
```
...
이러한 데이터를 처리하기 위한 핸들러 함수는 다음과 같습니다.

```tsx
const handleSegmentLock = (data: WebSocketData) => {
  if (data.UpdateSegment) {
    const { account_id } = data;
    const { segment_id } = data.UpdateSegment;

    // 이전에 해당 유저가 점유했던 세그먼트가 있다면 해제
    setSegmentOwners((prev) => {
      const newOwners = new Map(prev);
      const prevSegmentId = lockedSegments.get(account_id);
      if (prevSegmentId) {
        newOwners.delete(prevSegmentId);
      }
      newOwners.set(segment_id.toString(), account_id);
      return newOwners;
    });

    // 유저의 새로운 세그먼트 점유 설정
    setLockedSegments((prev) =>
      new Map(prev).set(account_id, segment_id.toString()),
    );
    setActiveUsers((prev) => new Set(prev).add(account_id));
  }
};
```

이러한 Map 구조를 통해 아래와 같은 기능들을 효율적으로 구현할 수 있었습니다.

##### 1. 세그먼트 점유 상태 확인 (O(1))

```tsx
const isSegmentLocked = (segmentId: string): boolean => {
  return segmentOwners.has(segmentId);
};

const getSegmentOwner = (segmentId: string): string | undefined => {
  return segmentOwners.get(segmentId);
};
```
...
```tsx
const getUserLockedSegment = (accountId: string): string | undefined => {
  return lockedSegments.get(accountId);
};
```
...
```tsx
const handleSegmentUnlock = (accountId: string) => {
  const segmentId = lockedSegments.get(accountId);
  if (segmentId) {
    setLockedSegments((prev) => {
...
이를 위해서 기존에는 `UpdateSegment` 이벤트를 받을 때마다 업데이트된 세그먼트들로 `segments` 상태를 업데이트했습니다.

```tsx
const handleSegmentUpdate = (data: UpdateSegmentData) => {
  const { segment_id, content } = data.UpdateSegment;

  setSegments((prevSegments) =>
    prevSegments.map((segment) =>
      segment.id === segment_id ? { ...segment, content } : segment,
    ),
  );
};
```

이러한 방식은 다음과 같은 문제가 있었습니다:

1. O(n)의 시간복잡도: 매번 전체 세그먼트 배열을 순회함
2. 불필요한 리렌더링: 하나의 세그먼트 변경으로 전체 배열 재생성
3. 빈번한 상태 업데이트: 실시간 이벤트마다 즉시 setState 호출

이러한 문제를 해결하기 위해 Map 기반의 상태 구조로 개선했습니다.

```tsx
const [segmentsMap, setSegmentsMap] = useState<Map<string, Segment>>(new Map());

const updateSegment = (segmentId: string, content: string) => {
  setSegmentsMap((prev) => {
    const newMap = new Map(prev);
    const existingSegment = newMap.get(segmentId);
    if (existingSegment) {
      newMap.set(segmentId, { ...existingSegment, content });
    }
    return newMap;
  });
};
...
여기에 추가로, 서버는 클라이언트가 보내는 데이터를 다른 유저들에게 전부 relay하고 있기 때문에 각 유저가 세그먼트를 수정하는 이벤트에 대해 디바운싱을 적용하여 각 유저의 입장에서
한 번에 너무 많은 데이터를 수신하지 않도록 조절했습니다.
