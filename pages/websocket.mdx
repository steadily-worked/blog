...
const [lockedSegments, setLockedSegments] = useState<Map<string, string>>(new Map());

// 세그먼트별 점유자 매핑 (segmentId -> accountId) - 빠른 조회용
const [segmentOwners, setSegmentOwners] = useState<Map<string, string>>(new Map());
```
...
const handleSegmentLock = (data: WebSocketData) => {
  if (data.UpdateSegment) {
    ...
    // 유저의 새로운 세그먼트 점유 설정
    setLockedSegments(prev => new Map(prev).set(account_id, segment_id.toString()));
